...
```{r}
  scan_filter <- function(data) {
    data |>
      filter(scan_datetime > lubridate::ymd("2022-01-01")) 
  } 

  measurement_filter <- function(data) {
    data |>
      filter(file_datetime > lubridate::ymd("2022-01-01"))
  }
```
...
Somehow, s44_init and r44_init are always NA now...
```{r}
  tar_read(motu_temperature) |>
    measurement_filter() |> 
    ggplot(aes(x = file_datetime, y = init_int, 
